...
import { useWallet, WalletName } from '@pontem/aptos-wallet-adapter';
...
const ConnectWalletButton: React.FC = () => {
  ...
  const fetchBalance = useCallback(async () => {
    if (!wallet?.adapter || !account?.address || !network) return;
    
    setLoadingBalance(true);
    try {
      const name: string = network.name || '';
      if (name.toLocaleLowerCase().includes('cedra')) {
        ...
        if (!apiUrl) {
          throw new Error('Network API URL not available');
        }
        // const response = await fetch(`https://api.opensea.io/api/v1/assets?collection=cryptopunks`);
        // // const response = await fetch(`${apiUrl}/accounts/${account.address}/balance/0x1::cedra_coin::CedraCoin`);
        ...
        // const balanceInOctas = parseInt(data.data.coin.value);
        // const balanceInAPT = (balanceInOctas / 100_000_000).toFixed(4);
        // setBalance(`${balanceInAPT} APT`);
      } else {
  ...
  }, [wallet?.adapter, account?.address, network]);
  ...
  const getWalletIcon = (wallet: any): string => {
